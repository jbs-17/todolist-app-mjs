...
try {
  LOGGER.info({
    message: "mencoba membuat pooling ke database...",
    label: "db",
  });
  ...
  LOGGER.error({
    message: "terjadi error pada saat membuat koneksi pooling ke db !",
  });
  console.error(error);
  process.exit(1);
} finally {
...
const gracefulShutdown = (event) => {
  LOGGER.info({
    label: "db",
    message: "Proses menerima ${ event }. Melakukan pembersihan...`",
  });
  ...
  if (event === "SIGINT" || event === "SIGTERM") {
    LOGGER.info({
      label: "db",
      message: "Pembersihan selesai. Keluar sekarang.",
    });
...
process.on("unhandledRejection", (reason, promise) => {
  LOGGER.error({
    label: "db",
    message: "Penolakan promise tak tertangani pada:",
    promise,
    reason,
  });
...
process.on("exit", (code) => {
  LOGGER.info({
    label: "db",
    message: `Proses keluar dengan kode: ${code}`,
    code,
  });
